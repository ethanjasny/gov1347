...
library(tidyverse)
setwd("~/Dropbox/ElectionAnalytics_Midterms/Lab sessions/Polling/Section data")
poll_df <- read_csv("~/Dropbox/ElectionAnalytics_Midterms/Lab sessions/Polling/Section data/polls_df.csv") # replace with your own wd() 
# transform poll date to date class
poll_df$poll_date <- as.Date(poll_df$poll_date, format = "%m/%d/%Y")
```
...
```{r, echo=FALSE, eval=TRUE}
library(zoo)
poll_df$seven_day_rolling <- rollmean(poll_df$avg_support, k = 20, fill = NA)


poll_df %>%
  filter(year == 2018, poll_date != "2018-12-09") %>%
  group_by(party) %>%
  summarise(seven_day_rolling <- rollmean(avg_support, k = 7, fill = NA)) %>%
  ggplot(aes(x = poll_date, y = seven_day_rolling,
             colour = party)) +
  geom_point(size = 0.3) +
  geom_line(size = 0.3) +
  scale_x_date(date_labels = "%b, %Y") +
...
popvote_df <- read_csv("~/Dropbox/ElectionAnalytics_Midterms/Lab sessions/Polling/Section data/H_popvote_df_fin.csv") # replace with own wd()
poll_df <- read_csv("~/Dropbox/ElectionAnalytics_Midterms/Lab sessions/Polling/Section data/polls_df.csv")
