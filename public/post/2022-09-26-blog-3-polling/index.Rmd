...
```
<br/>
Both models predict very similar seat counts for both parties, suggesting that PVI with adjustments for the national environment may be a decent stand-in for district-level polling in unpolled districts. Shown below are histograms plotting the distribution of the predicted district partisanship for the two models. (Note that a district partisanship value of 20, means we'd expect Democrats to to win roughly 50 + 20 = 70% of th two-party vote.) Plotted on the bottom is simply the distribution of baseline PVI for all congressional districts, without adjusting for the national environment.
```{r}
