...
<iframe height="800" width="100%" frameborder="no" src="https://ethanjasny.shinyapps.io/Histograms/"> </iframe>
...
plot_ly(x = dem_wins, type = "histogram", marker = list(color = 'blue', opacity = 0.5), name = "Simulated Dem Seat Share") %>% 
...
ui <-shinyUI(fluidPage(selectInput("selectPlot", "Choose desired congressional district", choices=merge_point$District, plotlyOutput("plot"))))

server <- shinyServer(function(input,output){      
  output$plot <- renderPlotly({
    return(get(merge_point$districtlabs[1]))
  })
})

merge_point$District == input$selectPlot
server <- shinyServer(function(input,output){      
  output$plot <- renderPlotly({
    return(get(plot_list[[which(merge_point$District == input$selectPlot)]]))
  })
})

p <- merge_point[1,]$plots

shinyApp(ui,server)

merge_point$plots <- plot_list

string <- merge_point$districtlabs

AL01 <- plot_list[[1]]

which(merge_point$District == input$selectPlot)

get(merge_point$districtlabs[1])
